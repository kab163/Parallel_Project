...
__global__ void fractal(unsigned char *d_cnt, int width, int height, int maxdepth, double dx, double dy, int from)
{
  ...
  if (index < (width * height)) {
    /* compute fractal */
    row = (index / width) + from; //compute row #
...
{
  double dx, dy;
  int width, maxdepth, groups, height;
  unsigned char *cnt;
  ...
  /* check command line */
  if(argc != 4) {fprintf(stderr, "usage: exe <width> <depth> <num groups>\n"); exit(-1);}
  width = atoi(argv[1]);
  ...
  groups = atoi(argv[3]);
  if (groups < 0) {fprintf(stderr, "number of groups should be at least 1\n"); exit(-1);}
  ...
  if(width % groups != 0) {fprintf(stderr, "number of groups must evenly divide width\n");}
  height = width / groups;
  ...
  cudaMalloc((void**)&d_cnt, width * height * sizeof(unsigned char));
  gettimeofday(&start, NULL);
  
  for (int i = 0; i < groups; i++) { 
    fractal<<<((width * height + THREADS-1) / THREADS), THREADS>>>( d_cnt, width, height, maxdepth, dx, dy, i * height);  
    cudaMemcpyAsync(&cnt[(i * height) * width], d_cnt, width * height * sizeof(unsigned char), cudaMemcpyDeviceToHost);    
  }
